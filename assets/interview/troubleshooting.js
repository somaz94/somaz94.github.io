...
(function () {
  ...
  /* id → the text of the card it names on the PARENT page. Built from the same
     decrypted bank this page already holds, so a ref can show WHAT it opens
     instead of an opaque slug the reader has to click to identify. */
  var LOOKUP = {};

  function buildLookup(bank) {
    LOOKUP = {};
    (bank.questions || []).forEach(function (q) { LOOKUP[q.id] = q.q; });
    (bank.cs || []).forEach(function (q) { LOOKUP[q.id] = q.q; });
    (bank.terms || []).forEach(function (t) { LOOKUP[t.id] = t.term; });
  }
  ...
  function buildItem(item, layerKey, labels) {
    ...
    /* `refs` names a card on the parent page — 55 of the 59 are CS questions.
       It has been authored and test-checked since this bank was written and
       rendered NOWHERE until now, which is the same as having no link at all:
       the reader diagnosing something had no way to reach the knowledge card
       that explains the mechanism they just applied. A ref whose id no longer
       resolves is dropped rather than rendered dead — the test keeps that from
       happening, this keeps it from showing if it ever does. */
    var refs = (item.refs || []).filter(function (r) { return LOOKUP[r]; });
    if (refs.length) {
      var box = node('p', 'ts-refs');
      box.appendChild(node('span', 'ts-refs-label', '관련 지식'));
      refs.forEach(function (r) {
        var a = node('a', 'ts-ref', LOOKUP[r]);
        a.href = '/interview/#' + encodeURIComponent(r);
        box.appendChild(a);
      });
      li.appendChild(box);
    }
    ...
    ROWS.push({
      ...
      hay: [].concat(item.title, item.symptom, item.check, item.fix || '',
                     item.note || '', item.code || '',
                     refs.map(function (r) { return LOOKUP[r]; }))
