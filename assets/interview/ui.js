...
(function () {
  ...
  /* `refs` targets, by id. `Object.create(null)` for the reason the tools file
     already records: a plain literal answers `x['constructor']` with a truthy
     function, and a lookup miss that returns something truthy renders as a
     silently wrong row rather than as nothing. */
  ...
  function followText(list) {
    ...
    /* The answers carry newlines now that they render as paragraphs, and the
       haystack is matched with indexOf — so a QUOTED phrase that happens to
       straddle a paragraph break would stop matching, silently and only for
       that one query shape. Collapsing whitespace keeps the search behaving
  ...
  function followBlock(q) {
    ...
    q.follow_ups.forEach(function (f) {
      ...
      d.appendChild(sum);
      /* Paragraphs, split on the newline exactly as the draft and the script
         bank are — the answers here run to several hundred characters and a
         single block is a wall the reader re-parses on every open. The BORDER
         stays on the container rather than moving to each paragraph: one <p>
         per rule would break the vertical line into segments with gaps, which
  ...
  /* `refs` — the card's links into the OTHER banks. Two shapes on purpose:

     A GLOSSARY term opens IN PLACE. Its definition is one line plus a short
     explanation, so moving the reader to another bank to see it would cost the
     card they are in the middle of for something that fits under the question.

     A CS card NAVIGATES, because it is a whole card to read. That reuses the
     deep-link path the hash already takes — bank switch, filters cleared,
     scroll, flash — and pushes history so the browser's Back returns to the
     card the reader left. Without the pushState the reader lands on a card
     they cannot get back from except by re-finding it.

     A ref whose id no longer resolves is DROPPED rather than rendered dead. The
     test keeps that from happening; this keeps it from showing if it ever does. */
  /* The ref labels are ON SCREEN, so they belong in the haystack — the page's
     standing rule that what the reader can see is what a search can reach. A
     term's `detail` is deliberately left out: it is behind a disclosure and the
     glossary bank already carries it in its own row's haystack. */
  function refsText(q) {
      ...
      if (r.kind === 'term') {
        ...
        a.addEventListener('click', function (e) {
          ...
          if (history.pushState) {
            /* Stamp the CARD THE READER IS ON into the current entry before
               pushing the target. Without it Back returns to an entry with no
               hash, `openDeepLink` finds nothing and returns false, and the view
               stays on the card they were trying to leave — the button looks
               broken, which is worse than having no history at all. */
  ...
  function buildRow(q, index) {
    ...
    /* The 성격 chip, and the axis the card is colour-coded by. It sits where the
       lens chip used to and reads the same way — what KIND of question this is,
       before which topic it is about. */
    if (hasKind()) {
  ...
  /* Same gate, same reason, second axis: a chip row with one button answers
     nothing. `kinds:` is the 성격 axis — 왜 / 어떻게 / 장애 / 가정 / 검증 —
     and it is deliberately NOT the lens. A lens is who is asking; a kind is what
     is being asked, which is the axis the bank is actually organised on. */
  ...
  /* Built ONCE, for the reason spelled out above `renderLensChips` — rebuilding
     a toggle row inside its own click handler removes the button the reader just
     pressed Enter on, and focus falls to <body>. The pressed state is written by
     `syncToggles` and nowhere else. */
  ...
  function render(data) {
    ...
    /* Lookups for `refs`. Built once here rather than inside the render
       function, which would rebuild them for every one of ~500 rows. */
    ...
    /* Move focus out of the now-hidden form, or a keyboard reader is left on a
       control that no longer exists. A deep link takes that focus instead — it
       has its own target and stealing it back to the search box would scroll
       away from the card the link just opened. */
    if (!openDeepLink()) el.q.focus();

    /* The other half of the pushState above. Back and Forward move the history
       entry; nothing re-reads the hash unless this does. */
    window.addEventListener('popstate', function () { openDeepLink(); });
  }

  /* A deep link from /interview/troubleshooting/, shaped `/interview/#<id>`.
     Every card here is behind the passphrase, so this cannot run before the
     bank is built — which is why it hangs off the unlock rather than off load.

     It clears the filters and switches banks BEFORE looking for the row. The
     target is usually in a bank the reader is not on, and any filter still set
     would hide it; either way the page would render the link as "조건에 맞는
     항목이 없습니다", which reads as a broken link rather than as a filter. */
  function openDeepLink() {
    ...
    /* Focus, not just scroll: a screen reader is otherwise still at the top of
       a page that silently moved. The row is not a control, so it borrows a
       tabindex — `-1` keeps it out of the Tab order the palette-style roving
       index depends on. */
