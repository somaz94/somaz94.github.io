...
(function () {
  ...
  function applyLanguage(next, restoring) {
    ...
    /* The placeholder is an attribute too, and the text swap above never
       reaches it. Written as a sweep rather than against `searchInput` alone
       because the password gate has a field as well, and a second special case
       is how the first one stops being noticed. */
  ...
  /* The same lock /resume/ and /career/ carry, and the same password. What it
     is worth is worth stating: `atob` hides a value from a reader who is not
     looking, and View Source defeats it in one step. This stops the document
     being lifted in passing; it is not a claim that the text is protected.
     /interview/ is where this repo does the real thing, and it can only do it
     because that page's content is ciphertext until a key opens it.

     The lock itself is `body.content-locked`, set in the markup so it holds
     before this file runs — a lock a blocked script leaves open is not one.
     Everything below only offers the way past it: a page that silently refuses
     to be selected reads as broken. */
  ...
  function checkGate() {
    /* The password comes off `data-k`, written there by Liquid from
       _data/private/gate.yml — the one place it is spelled. This file is served
       as a static asset and never sees Liquid, so a literal here could not read
       that file and would be a second copy of the same secret; the two would
       then disagree the first time one of them was edited, with nothing to say
       which page still answered to the old value. An absent attribute is
       treated as a wrong password rather than as no gate. */
    ...
    closeGate();
    /* The lock lifting is invisible — nothing on screen changes — so it is
       announced through the status line the page already has rather than with
       a toast this page has no other use for. */
  ...
  function guardContent(ev) {
    if (unlocked) return;
    /* The password field is inside the gate and has to stay usable, and so does
       the search box — a filter nobody can type into is a broken page. */
  ...
  if (gate) {
    ...
    /* Drag detection is MOUSE-ONLY. A touch `pointermove` after a `pointerdown`
       is a scroll, and prompting on it would throw the gate up on every swipe
       down the page. */
    ...
    document.addEventListener('pointermove', function (ev) {
      ...
      /* 24px rather than a few: a click that jitters is not a drag. Cleared as
         it fires, so one prompt per gesture. */
