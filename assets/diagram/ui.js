...
(function (global) {
  ...
  /* Shared with the other tools; the old `dg-theme` is adopted once and deleted.
     Unlike them this page defaults to LIGHT, not the OS: dark chrome around the
     diagram's white canvas is a worse first impression. */
  var THEME_KEY = 'tool-theme';
  var LEGACY_THEME_KEY = 'dg-theme';
  var HELP_KEY = 'dg-help-seen';

  /* Shortcuts accept Ctrl or Meta already; this only names them for the host.
   * The hints are joined, not `||`-chained: Chrome reports a truthy lower-case
   * "macOS" (headless: ""), so a first-match chain would stop there. */
  ...
  /* Read off the root, never re-derived: the pre-paint script in `diagram.html`
   * decides, so the stylesheet and the behaviour cannot disagree. */
  ...
  /* A small index plus one key per diagram, so boot reads the index and one
   * model. The legacy `dg-diagram-v1` slot is migrated once and removed, or the
   * next boot would have two candidates for "your last diagram". */
  ...
  function writeStore(key, value) {
    try {
      ...
      /* Private mode or a full quota: the editor works but will not resume.
       * Say so once, not on every keystroke. */
      if (!quotaWarned) {
        quotaWarned = true;
  ...
  /* The legacy key is dropped only once the copy is written, so a failed write
   * leaves the old diagram in place. The migrated one never steals focus. */
  ...
  /* Every entry switch ends at `adoptLoadedModel()`, the single place that
   * brings the editor into line with a swapped-in model. */
   ...
   * entry; nothing open is ever overwritten. */
  ...
  /* For sets we may not redistribute (Azure): the user brings their own SVGs
   * and they never leave this browser. IndexedDB, because a vendor pack would
  ...
  function paletteItem(icon) {
    ...
    /* Roving tabindex: the palette is one tab stop and the arrow keys move
     * inside it; `paint()` marks exactly one tile `0`. */
    btn.tabIndex = -1;
    ...
    /* A button cannot contain a button, so the remove control is a sibling in a
     * positioned wrapper, which also keeps it off the tile's drag path. */
    var wrap = document.createElement('div');
  ...
  /* Attribution is a licence condition for the Kubernetes set — do not drop it.
   * Groups may cite several sets and share them, so this de-duplicates by text. */
  ...
  /* Keyboard traversal is in reading order, not z-order: "the next one down the
   * page" is what someone stepping through a diagram expects. */
  ...
  /* On a coarse pointer ports and handles grow their HIT area; the drawn dot
   * stays put. Handles grow less: a corner and its neighbouring side port are
   * only half a node apart and would fight over every tap. */
  ...
  function addPorts(layer, node, visible) {
    ...
    PORT_SIDES.forEach(function (side) {
      ...
      // A transparent disc under the small dot keeps it grabbable at any zoom.
      var hit = el('circle', {
  ...
  /* Drags listen on `window`, not setPointerCapture: capture is silently lost
   * when the captured element re-renders mid-drag (the palette repaints on every
   * search keystroke), and the drop never commits. */
  ...
  /* Touch pan and pinch live here: `touch-action: none` lets a node drag without
   * the page scrolling, but hands us the pinch too. Not `bindDrag`, because a
   * one → two → one finger gesture must outlive the pointer that started it. */
  ...
  function onDoubleClick(ev) {
    ...
    /* Edges rename on double-click too. Hit-test the overlay's fat path; the
     * drawn line is too thin to hit reliably. */
  ...
  /* A textarea over the canvas, not a `<foreignObject>`, which does not survive
   * PNG rasterisation. The box is measured from the rendered `<text>` so it lands
   * on the glyphs; with no label yet it falls back to where one would render. */
  var LABEL_STYLES = {
  ...
  /* Nodes and edges share this editor; an edge has no `type`. An unlabelled edge
   * is placed off its drawn path with `getPointAtLength`, which is right
  ...
  function beginLabelEdit(node) {
    ...
    /* iOS Safari zooms the viewport when a focused control computes below 16px
     * and never zooms back. So compute 16px and scale the box down by the same
     * factor (width and height too): the glyphs still track the label and zoom.
     * Coarse pointers only, since the scale costs some text sharpness. */
  ...
  function endLabelEdit(revert) {
    ...
    edit.el.remove();
    /* Focus returns to the canvas, or a keyboard rename ends on `<body>` and Tab
     * restarts at the top. Only if nothing else claimed it (a toolbar button). */
  ...
  function inspectMany(host, nodes) {
    ...
    /* `edge` picks the extreme to snap to; `centre` averages the span so the row
     * lands on its own midline. */
  ...
  function inspectNode(host, node) {
    ...
    // Text has its own colour, so name what this one actually drives.
    ...
    actions.className = 'dg-actions';
    if (node.type !== 'group') {
      // Array order is z-order, so both of these are one splice.
  ...
  /* Auto/Custom is a segmented pair, not a bare picker, so "follow the accent
   * colour" stays reachable after a custom one is set; picking implies Custom. */
  ...
  /* Falls back rather than emitting `rgba(NaN,…)`: a group's colour comes off
   ...
   *
   * The optional sides pin the anchors: routing has no obstacle avoidance.
   */
  var TEMPLATES = [
    {
      key: 'aws-3tier',
  ...
  function loadFromHash() {
    ...
    // Returned, not installed: a share link opens as its own library entry, so
    // it must not touch `state.model` on the way past.
  ...
  /* Rewrites the Windows-authored modifier names in the dialog `<kbd>`s and the
   * toolbar tooltips; the inspector's list builds from the same constants. */
  ...
  /* Shown unprompted once: side dots are the only way to connect and group
   * presets appear only on a selected group, both hard to stumble onto. */
  function openHelp() {
  ...
  function bindCanvas() {
    ...
    /* Bound once, not per gesture: fingers arrive and leave independently. Each
     * returns at once for a pointer the touch layer is not tracking. */
