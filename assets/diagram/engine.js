...
(function (global) {
  ...
  /* icons.js is only an index; path data is split per group and fetched on
   * first use. Until its chunk arrives an icon has `body === null`: callers
   * wait on `ensure()` or draw the placeholder. */
  var Icons = (function () {
    ...
    function init() {
      ...
      groups = data.groups.map(function (g) {
        var icons = g.icons.map(function (i) {
          var icon = {
            ...
            group: g.id,
            // Per icon as well as per group: the ecosystem set mixes brand
            // logos with single-path simple-icons that follow the node colour.
            mono: !!(i.m || g.mono)
          };
          byKey[icon.key] = icon;
          return icon;
        ...
        var group = {
          ...
          chunk: g.chunk || '',
          loaded: !g.chunk,
          // A list: one group can draw from more than one upstream set.
      ...
      // Always present so the empty tab can explain itself: it is the only
      // route for sets we may not ship (Azure). No credits: the files are the user's own.
      customGroup = {
  ...
  function makeNode(type, props) {
    ...
    // Empty `textColor` means "track `color`", so diagrams drawn before the
    // fill/text split render unchanged.
  ...
  /* Becomes each shape's SVG `<title>`: its accessible name and tooltip, in the
   * editor and in every exported SVG. */
  ...
  /* A `<title>` names its element only as the FIRST child, so it is inserted
   * ahead of the already-drawn shapes rather than appended. */
  ...
  function buildExportSvg(model, contentLayer, defsLayer, opts) {
    ...
    /* Named at the root too, so an embedded export announces itself as one
     * picture rather than a pile of unlabelled shapes. */
