 ...
 * Rule table. Adding a check means adding an entry here, not editing inspect.js.
 *
 * Every finding names a release or a concrete setting the reader can check; a
 * rule that guesses is worse than a rule that is absent.
 *
 * Deliberately out of scope: manifest diffing, per-rule on/off, share-by-link.
 */
(function (global) {
  'use strict';

  /* Keyed by "<apiVersion>/<kind>": the same group/version was removed for some
   * kinds and kept for others. `removedIn` is the release that stopped serving it. */
  var DEPRECATED = {
    // Removed in 1.16
  ...
  var CHECKS = [
    {
      ...
      /* Without this, a missing or misindented pod template makes every
       * container rule return nothing, which `inspect` counts as passed. */
      test: function (doc) {
        var spec = podSpecOf(doc);
    ...
    },
    {
      ...
      test: perContainer(function (c) {
        ...
        /* A digest carries no stale-cache risk. Must precede `tagOf`, which reads
         * the colon in `@sha256:` as a tag and would contradict `unpinned-image`. */
    ...
    },
    {
      ...
      /* The one rule about the whole input, which is why every test also
      ...
      test: function (doc, all) {
        ...
        /* A PDB only covers its own namespace; matching on kind alone would pass
         * prod on staging's PDB. Selector matching is not attempted; the detail
         * says so. */
